        ...
        <div className='bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg'>
          ...
          <ul className='space-y-4'>
            {recentActivities.map(activity => (
              <li key={activity.text} className='flex items-start'>
        ...
        <div className='bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg'>
          ...
          <ul className='space-y-4'>
            {lowStockAlerts.map(alert => (
              <li key={alert.text} className='flex items-start'>
                <div
                  className={`p-2 rounded-full mr-4 mt-1 ${alert.critical ? 'bg-red-500/20' : 'bg-yellow-500/20'}`}
                >
                  <ExclamationIcon
                    className={`w-5 h-5 ${alert.critical ? 'text-red-500 dark:text-red-400' : 'text-yellow-500 dark:text-yellow-400'}`}
                  />
                </div>
                <div>
