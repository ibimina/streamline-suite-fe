...
import { Bar, BarChart, ResponsiveContainer, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, PieChart, Pie, Cell } from 'recharts';
import { CurrencyDollarIcon, PresentationChartLineIcon, UsersIcon } from './Icons';

const kpiData = [
  { title: "Total Revenue", value: "$525,300", change: "+15.2% vs last period", icon: <CurrencyDollarIcon className="w-7 h-7 text-gray-400 dark:text-gray-500"/> },
  { title: "Net Profit", value: "$128,450", change: "+8.8% vs last period", icon: <PresentationChartLineIcon className="w-7 h-7 text-gray-400 dark:text-gray-500"/> },
  { title: "Avg. Sale Value", value: "$4,825", change: "+$250 vs last period", icon: <UsersIcon className="w-7 h-7 text-gray-400 dark:text-gray-500"/> }
];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Sales by Service/Product</h3>
                     <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={salesByServiceData} layout="vertical" margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                             <CartesianGrid strokeDasharray="3 3" stroke="#4B5563" />
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg flex flex-col items-center">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 self-start">Top Customers by Revenue</h3>
                    <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                            <Pie data={topCustomersData} cx="50%" cy="50%" outerRadius={100} fill="#8884d8" dataKey="value" nameKey="name" labelLine={false} // Fix: Add explicit `any` type for label renderer props to resolve library typing issue.
